...
inline __device__ int32_t reduce_max_shuffle(int32_t val) {
    ...
    #pragma unroll
    for (int offset = GSPLAT_HIP_WARP_SIZE / 2; offset > 0; offset /= 2) {
...
template<int LOGICAL_WARP_SIZE = GSPLAT_HIP_WARP_SIZE>
...
template<int N, int LOGICAL_WARP_SIZE = GSPLAT_HIP_WARP_SIZE>
...
inline __device__ void manual_dynamic_reduce_sum_vec2(  
    ...
    for (int i = 0; i < GSPLAT_HIP_WARP_SIZE; ++i) {
